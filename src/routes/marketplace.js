...
router.get('/', async (req, res) => {
    ...
    const tickets =
				[...(gql_response.data.data.user.ticketsOwned.items ?? []), ...(gql_response.data.data.listed.items ?? [])];

			// Extract eventIds from tickets
			const eventIds = tickets.map(
				(ticket) => ticket.eventId.split('-')[1]
			);
    ...
    listedTickets.forEach((listing) => {
      if (!marketplaceTickets[listing.eventId.split('-')[1].toLowerCase()]) {
        marketplaceTickets[listing.eventId.split('-')[1].toLowerCase()] = [];
      }
      if (listing.seller.address.toLowerCase() === account.toLowerCase()) {
        return;
      }
      marketplaceTickets[listing.eventId.split('-')[1].toLowerCase()].push({
